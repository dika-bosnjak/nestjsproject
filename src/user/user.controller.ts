import { Body, Controller, ExecutionContext, Get, HttpCode, HttpStatus, Param, ParseIntPipe, Patch, Post, Query, Req, UseGuards, ValidationPipe } from '@nestjs/common';
...
export class UserController {
    ...
    @ApiOkResponse({type: Token, description: '200. Success.',})
    ...
    @ApiParam({ name: 'firstName', type: String, required:false })
    @Get()
    getUsers(@Query('firstName') firstName?:string): any {
        return this.userService.getUsers(firstName)
    }
